<template>
  <div class="not-prose mt-12">
    <h2 class="text-2xl font-semibold tracking-tight text-gray-900 sm:text-3xl dark:text-white mb-4">{{ date }}</h2>
    <ol class="relative border-l border-gray-200 dark:border-gray-700 mb-10">
      <li v-for="event in events" :key="event.title" class="mb-10 ml-4">
        ...
        <time class="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-400">{{event.time }}</time>
        <h3 class="text-lg font-semibold tracking-tight text-gray-900 sm:text-xl dark:text-white">{{ event.title }}</h3>
