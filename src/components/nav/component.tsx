...
export default function Nav({ data }: { data: NavQuery }) {
    ...
    return (
        <>
            {data?.siteInfo?.nav?.banner?.showBanner && (
                <div
                    className={cn(
                        ...
                        bannerBackgrounds[
                            data.siteInfo.nav.banner.type as bannerStatus
                        ],
                    )}
                >
                    {getBannerIcon(
            ...
            >
                {/* first nav pill */}
                <div className="px-5 sm:px-4 rounded-full mt-4 bg-slate-900/50 backdrop-filter backdrop-blur-[2px] button gap-x-4 sm:gap-x-2 flex items-center">
                    {/* logo */}
                    <a href="/">
                        <img
                    ...
                    <div className="sm:hidden flex">
                        <Sheet>
                            ...
                            <SheetContent>
                                <div className="px-5 flex flex-col justify-center h-full text-3xl gap-y-2 font-medium relative bg-slate-800">
                                    {Links()}
                                </div>
                            </SheetContent>
                        </Sheet>
                ...
                {data?.siteInfo?.nav?.featured?.showFeatured && (
                    ...
                    >
                        <div className="px-4 pr-2 rounded-full mt-4 bg-slate-900/50 hover:bg-slate-800/50 transition backdrop-filter backdrop-blur-[2px] button flex items-center gap-x-3">
                            <p>{data.siteInfo.nav.featured.message}</p>
                            <div className="bg-orange-500 rounded-full p-1 px-1.5 transition flex gap-x-2 items-center font-semibold">
